...
Module-import-time coverage wiring (parallel + subprocess
support): `os.environ.setdefault` would be a no-op here
because pytest-cov has already set COVERAGE_FILE to a tmp
dir by the time conftest is loaded, so we force-set.
"""
...
def _ensure_subprocess_coverage_shim() -> None:
    ...
    pth = purelib / "_scitex_session_subprocess_coverage.pth"
    shim = (
        "import os, coverage\n"
