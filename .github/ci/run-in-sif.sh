...
# the shared ci-cpu.sif bakes scitex-dev[all,dev] DEPS, NOT scitex-session's —
...
# so only scitex-session's own thin dep set is fetched per run.
#
...
export TMPDIR="/tmp/ci-scitex_session-${GITHUB_RUN_ID:-0}-${GITHUB_RUN_ATTEMPT:-0}-$V"
rm -rf "$TMPDIR"
...
# scitex-session's reproducibility tests (validate_recipe renders the SAME recipe
...
# Install scitex-session + its [all,dev] extras WITH deps into the writable target.
# Fallback chain mirrors scitex-session's historical bare-uv/pip workflow so a
...
# scitex-session's suite is ~2460 tests; single-process it overran the job's old
...
# scitex-session style-stack are naturally isolated per worker — the safe way to
# parallelise a matplotlib-heavy suite.
#
# Worker count: use ALL cores. Each matrix leg now runs on its own dedicated
# self-hosted node (one runner per node: scitex-session-01/02/03), so there is no
...
# loadscope pins an entire MODULE's tests to ONE worker — and scitex-session's heavy
...
exec nice -n 19 ionice -c 3 \
    ...
    --cov=src/scitex_session --cov-report=xml --cov-report=term \
    -p no:cacheprovider
